...
const teacherRegister = async (req, res) => {
    ...
    try {
        ...
        result.password = undefined; // Don't send password in response
        res.send(result);
    } catch (err) {
    ...
    try {
        ...
        if (teachers.length > 0) {
            teachers = teachers.map(teacher => {
                teacher.password = undefined; // Don't send password in response
                return teacher;
            });
            res.send(teachers);
        } else {
    ...
    try {
        ...
        if (deletionResult.deletedCount > 0) {
            await Subject.updateMany(
                { teacher: { $in: deletionResult.deletedIds }, teacher: { $exists: true } },
                { $unset: { teacher: "" } }
            );
